...
export const handler = {
	async POST(req: Request, ctx: FreshContext) {
		...
		const response = await fetch(
			"https://mdy2rbcypyehddeuvi2s55k56i0mkqtm.lambda-url.us-east-1.on.aws/login",
			{
				method: "POST",
		...
		if (response.ok) {
			...
			setSession({ accessToken, refreshToken, expiresAt }, headers);
		...
		<>
			...
			<div class="flex items-center justify-center h-screen bg-gray-100">
				...
				>
					...
					{data?.errorMessage && (
						<div class="mb-4 p-2 bg-red-100 text-red-600 border border-red-400 rounded">
							{data.errorMessage}
						</div>
					)}
					<div class="mb-4">
